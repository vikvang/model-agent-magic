...
const Index = () => {
  ...
  const [selectedModel, setSelectedModel] = useStateWithStorage<ModelType>(
    "gregify_selected_model",
    "gpt4"
  );
  const [selectedRole, setSelectedRole] = useStateWithStorage<AgentRole>(
    "gregify_selected_role",
    "webdev"
  );
  const [prompt, setPrompt] = useStateWithStorage<string>("gregify_prompt", "");

  // Session state (no need to persist)
  ...
  const parseNormalResponse = useCallback((response: string) => {
    ...
    const improvedPromptIndex = response
      .toLowerCase()
      .indexOf("improved prompt");
    ...
    const fullRegexPattern =
      /(?:Enhanced prompt(?:\s*|:\s*|["""]?\s*))([^]*?)(?:(?:\n\n|\r\n\r\n)(?:Explanation(?:\s*|:\s*)|(?:-{3,}))([^]*))/i;
    ...
    const enhancedPromptMarker = response
      .toLowerCase()
      .indexOf("enhanced prompt");
    ...
    if (enhancedPromptMarker !== -1) {
      ...
      if (
        explanationMarker !== -1 &&
        explanationMarker > enhancedPromptMarker
      ) {
        ...
        const separatorMatch = response
          .substring(enhancedStart)
          .match(/\n(-{3,}|\*{3,}|\_{3,})\n/);
        if (separatorMatch) {
      ...
      const enhancedPrompt = response
        .substring(enhancedStart, enhancedEnd)
        .trim();
      let explanation = "";
      ...
      if (explanationMarker !== -1) {
        ...
        while (
          explanationStart < response.length &&
      ...
      for (const para of paragraphs) {
        if (para.trim().length > 20) {
          ...
          const explanation = response
            .substring(response.indexOf(para) + para.length)
            .trim();
  ...
  const sendToChatGPT = useCallback(
    (enhancedPrompt: string) => {
      ...
      try {
        navigator.clipboard
          ...
          .then(() => {
            ...
            showToast(
              "Enhanced prompt copied to clipboard! Switch to ChatGPT tab and paste."
            );
          })
          .catch((err) => {
            console.error("Error copying to clipboard:", err);
            showToast(
              "Failed to copy to clipboard. Please use the Copy button manually."
            );
          });
      } catch (clipboardErr) {
        console.error("Clipboard API error:", clipboardErr);
        showToast(
          "Error accessing clipboard. Please use the Copy button manually."
        );
      }
  ...
  const handleGregify = async () => {
    ...
    if (selectedModel === "none") {
      setNormalResponse(
        "Error: No API keys available. Please add your API keys in your profile settings to use the application."
      );
      return;
    }
    ...
    if (!sessionId) {
      setNormalResponse(
        "Error: You must be logged in to use this feature. Please log in and try again."
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `Using AI Provider: ${aiProvider} for model: ${selectedModel}`
      );
      ...
      console.log("NORMAL mode response received, length:", response.length);
                  ...
                  {normalResponseParts.enhancedPrompt && (
                    <div className="p-4 bg-[#2C2C30] rounded-lg border-2 border-[#FF6B4A] relative group">
                      ...
                      <div className="mt-3 flex items-center justify-between text-xs text-zinc-400">
                        <span>
                          Enhanced prompt automatically sent to ChatGPT
                        </span>
                        <Button
                          variant="outline"
                          size="sm"
                          ...
                          onClick={() => {
                            ...
                            showToast(
                              "Copied to clipboard! You can paste in ChatGPT if needed."
                            );
                          }}
                        >
                          <svg
